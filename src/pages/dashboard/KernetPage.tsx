...
export default function KernetPage() {
  const [page, setPage] = useState(1);
  const [search, setSearch] = useState('');
  const utils = trpc.useUtils();

  const { data, isLoading } = trpc.employee.list.useQuery({ role: 'kernet', search: search || undefined, page, limit: 10 });
  ...
  return (
    <DataTable
      title="Manajemen Kernet"
      description="Kelola data kernet bus"
      columns={columns}
      ...
      addLink="/dashboard/kernet/tambah"
      editLinkPrefix="/dashboard/kernet"
      searchPlaceholder="Cari nama kernet..."
    />
  );
}
